...
type Context = { params: { ramo: string; anio: string; semestre: string; resourceId: string } };

export async function PATCH(request: Request, context: Context) {
  try {
    const roadmap = await requireRoadmap(parseCoursePath(context.params));
    const resourceId = requireUuid(context.params.resourceId, 'resourceId');
    ...
    if (Object.keys(data).length === 0) {
      return NextResponse.json({ error: { code: 'INVALID_REQUEST', message: 'Debe indicar al menos un campo para actualizar.' } }, { status: 400 });
    }
    const resource = await prisma.recurso.update({ where: { id: resourceId }, data });
    return NextResponse.json({ recurso: { id: resource.id, titulo: resource.titulo, url: resource.url, tipo: resource.tipo } });
  } catch (error) {
